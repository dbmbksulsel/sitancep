...
+--------------------------------------------------------+*/
$kv = new mysqli('localhost', 'root', '', 'sididok');
...
if(count($_FILES['attachment']['name']) > 0){
    ...
    $maxsize = 900 * 1024 * 1024 * 1024; //2MB maximum allowed.
    ...
    $allowed = array( 'jpg', 'jpeg', 'gif', 'png', 'bmp', 'pdf', 'doc', 'docx', 'JPG', 'JPEG', 'GIF', 'PNG', 'BMP', 'PDF', 'DOC', 'DOCX'    );
    ...
    foreach($_FILES['attachment']['name'] as $filename) {
      ...
      mysqli_query($kv, "INSERT INTO upload (pptk,filename, type, size, date) values ( '{$pptk}','".$_FILES["attachment"]["name"][$Kv]."', '".$_FILES["attachment"]["type"][$Kv]."', '".$_FILES["attachment"]["size"][$Kv]."', '".date('Y-m-d')."')");
